...
$scope.generisiIzvjestaj=function(){
		...
		if($scope.month==0){
			...
			doc.text('Broj rijesenih incidenata po mjesecima: '+$scope.Statistika.closedMonthly.join(' | '),20,193);
			doc.text('Broj aktivnih incidenata:'+$scope.Statistika.active,10,203);

		}else{

		doc.text('Broj aktivnih incidenata:'+$scope.Statistika.active,10,193);
	}

		if($scope.month==0){

		doc.text('Broj otvorenih incidenata po mjesecima:'+$scope.Statistika.allMonthly.join(' | '),20,213);
		doc.text('Broj pogresno prijavljenih incidenata:'+$scope.Statistika.falsePositive,10,223);
		doc.text('Odnos zatvoreni/otvoreni incidenti:'+(Math.round($scope.Statistika.closed/$scope.Statistika.all*100)/100),10,233);
		doc.text('Broj rjesenih incidenata po glavi uposlenika odgovorih odjela: '+ (Math.round($scope.Statistika.perWorker * 100) / 100)+' inc/upos',10,243);
		doc.text('------------------------------------------------------------------------------------------------------------',10,250);

		doc.text('Ukupan broj incidenata:'+$scope.Statistika.all,10,260);
		}
else{
		doc.text('Broj pogresno prijavljenih incidenata: '+$scope.Statistika.falsePositive,10,203);

		doc.text('Odnos zatvoreni/otvoreni incidenti: '+(Math.round($scope.Statistika.closed/$scope.Statistika.all*100)/100),10,213);

		doc.text('Broj rjesenih incidenata po glavi uposlenika odgovornih odjela: '+ (Math.round($scope.Statistika.perWorker * 100) / 100)+' inc/upos',10,223);
